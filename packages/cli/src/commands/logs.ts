...
import type { CliContext, ServiceName } from "@ploybundle/shared";
...
export function registerLogsCommand(program: Command, context: CliContext): void {
  program
    ...
    .option("--service <service>", `Filter by service: ${ALL_SERVICES.join(", ")}`)
    ...
    .action(async (projectName: string, options: Record<string, string>) => {
      ...
      try {
        ...
        const service = options.service as ServiceName | undefined;
        const logs = await adapter.fetchLogs(config.ssh, config, service);

        if (context.outputMode === "json") {
          output.json({ projectName, service: service ?? "all", logs });
        } else {
          output.log(logs);
        }
      } catch (err) {
