...
describe("CoolifyAdapter", () => {
  ...
  beforeEach(() => {
    ...
    adapter = new CoolifyAdapter(mockSsh as any);
  });
  ...
  describe("installPlatform", () => {
    it("skips if coolify already running and returns alreadyInstalled=true", async () => {
      mockSsh.exec.mockResolvedValue({
        stdout: "coolify",
        stderr: "",
        exitCode: 0,
    ...
    it("installs Coolify when not present", async () => {
      // First call: coolify not running
      mockSsh.exec
        .mockResolvedValueOnce({ stdout: "", stderr: "", exitCode: 0 })
        // Second call: curl install command succeeds
        .mockResolvedValueOnce({ stdout: "ok", stderr: "", exitCode: 0 })
        // Third+: waitForCoolify polling returns up
        .mockResolvedValueOnce({ stdout: "Up 10 seconds", stderr: "", exitCode: 0 });
      ...
      // Verify the curl-based install command was used
      expect(mockSsh.exec).toHaveBeenCalledWith(
        testSsh,
  ...
  describe("platformHealth", () => {
    it("returns healthy=true when coolify is up", async () => {
      mockSsh.exec.mockResolvedValue({
        stdout: "Up 2 hours",
        stderr: "",
        exitCode: 0,
      ...
      expect(health.message).toBe("Coolify is running");
    });

    it("returns healthy=false when coolify is down", async () => {
      mockSsh.exec.mockResolvedValue({
        stdout: "",
        stderr: "",
        exitCode: 0,
      ...
      expect(health.message).toBe("Coolify is not running");
    });
  });
  ...
  describe("deployStack", () => {
    it("uploads compose, env, configs and runs docker compose up", async () => {
      mockSsh.exec.mockResolvedValue({
        stdout: "running\nrunning",
        stderr: "",
        exitCode: 0,
      ...
      expect(result.message).toContain("via Coolify");
